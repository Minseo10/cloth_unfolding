...
class ImageViewer:
    def __init__(self, root, start_index=1, end_index=1000):
        ...
        for i in range(3):
            ...
            image_label.grid(row=0, column=i, padx=5, pady=5)  # 그리드 레이아웃에서 행 0, 열 i에 배치
            self.image_labels.append(image_label)

        # 경로 레이블 생성
        self.path_label = tk.Label(root, text="")
        self.path_label.grid(row=1, column=0, columnspan=3)

        # "다음" 버튼 생성
        self.next_button = tk.Button(root, text="다음", command=self.next_directory)
